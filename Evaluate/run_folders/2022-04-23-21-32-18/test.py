...
if __name__ == '__main__':
    ...
    env=opt.envname
    os.mkdir('/home/zhu_19/saved_mat/' + env)
    server='10.37.0.175'
    ...
    if conti == True:
        ...
        state_dict = torch.load('/home/zhu_19/evt_tracking/Evt_Swin_Back_Flow_n02/run_folders/2022-04-22-21-12-30/save_model2022-04-22-21-12-32/state_dicr_45.pkl' , map_location='cpu')
            ...
            for data_index in range(0,1):
                ...
                for name_i, Efame, GT in Edataset_loader:
                    ...
                    if name_i[0] != name_i_pre:
                        ...
                        if name_i_pre != None:
                            scio.savemat('/home/zhu_19/saved_mat/'+ env+'/'+ +name_i_pre +'PredBox.mat',{'PredBox':np.stack(saved_box,axis=0),'GTBox':np.stack(saved_Gtbox, axis=0),'IOU':np.stack(all_iou, axis=0)})
