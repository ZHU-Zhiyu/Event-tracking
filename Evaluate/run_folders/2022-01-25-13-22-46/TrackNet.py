...
class ETracking_Net(Module):
    def __init__(self,SwinT = None, bias = False):
        ...
        self.Regression_ipc = nn.Sequential(nn.Conv1d(1024, 32, kernel_size=1, bias=bias),
                                #    self.bnr,
                                   nn.LeakyReLU(negative_slope=0.1),
                                   nn.Conv1d(32, 1, kernel_size=1, bias=bias))
        ...
        self.classifier_physical = nn.Sequential(nn.Conv1d(6, 128, kernel_size=1, bias=bias),
    ...
    def _Backward_warp(self, X0):
        ...
        return torch.cat((project_pos,X0[:,2:4,:]), dim=1)
    ...
    def _point_cd_loss(self, X_warp, X0):
        ...
        loss_1 = loss_1 / (loss_thresh) * a.float()
        ...
        loss_2 = loss_2 / (loss_thresh) * a.float()
    ...
    def _weight_cal_box(self, Emb_X, prob_sem, prob_phy, X_pos):
        ...
        bbox_fused = self.Regression_layer(fea_fused)
        # bbox_fused in shape of B, 4, 2

        bbox_output = bbox_fused*confidence

        bbox_output = bbox_output.sum(dim=2,keepdim=True)
        # bbox_output in shape of B, 4, 2


        return bbox_output, bbox_fused

    def _weight_cal_box_test(self, Emb_X, prob_sem, prob_phy, X_pos):
        ...
        out_prob = torch.cat([prob_sem, prob_phy], dim = 1) * confidence.permute([0, 2, 1])

        return bbox_output, bbox_fused, out_prob.sum(dim = 1, keepdim = True)
