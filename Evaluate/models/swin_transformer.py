...
class Dynamic_sampling_M(nn.Module):
    def __init__(self,k=4, dilation = 1):
        super(Dynamic_sampling_M, self).__init__()
        self.k = 4

    def forward(self, x, s_num=None, Is_temp=False, Temp_ratio=0):
        ...
        PointSample = mesh
        # print('shape of x:{}, s_num:{}, length:{}'.format(x.shape, s_num, length))
        ...
        if C == 5:
            temp_k = 8
            indx = knn(x[:,:3,:],select_point[:,:3,:], temp_k )
        else:
            temp_k = 4
            indx = knn(x[:,:3,:],select_point[:,:3,:], temp_k )
        ...
        feature = gether_neibour(x, indx, s_num, temp_k)
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.sample_layer = Dynamic_sampling_M()
    ...
    def forward(self, x):
        ...
        # x = x.view(B, -1, 4 * C)  # B H/2*W/2 4*C
        x = self.sample_layer(x.permute(0,2,1))
        x_pos = x[:,:,0,:5]
        x = x[:,:,:,5:]

        B,P,N,C = x.shape
        # print('shape of X:{}, B:{}, P:{}, N:{}, C:{}'.format(x.shape,B,P,N,C))
        x = x.reshape([B,P,N*C])

        x = self.norm(x)
        x = self.reduction(x)

        output = torch.cat([x_pos, x], dim = 2)
        return output
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            x_ = self.downsample(torch.cat([x_pos,x], dim = 2))
        else:
            x_ = torch.cat([x_pos,x], dim = 2)
        return torch.cat([x_pos,x], dim = 2), x_
...
class SwinTransformer(nn.Module):
    ...
    def track_embedding(self, PNet_embedding):

        assert(len( PNet_embedding.shape ) == 3)

        x = self.pos_drop(PNet_embedding)

        Dnsamp_out = [x]
        output = [x]
        for layer in self.layers:
            x,x_ = layer(Dnsamp_out[-1])
            output.append(x)
            Dnsamp_out.append(x_)
        # B L C
        return Dnsamp_out, output
